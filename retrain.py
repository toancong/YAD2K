...
def _main(args):
    ...
    for (dirpath, dirnames, filenames) in walk(data_path):
        files = ['%s%s'%(dirpath, f) for f in filenames if f.endswith('.npz')]
    ...
    print('There are %s images %s boxes'%(len(images), len(boxes)))
...
def train(model, class_names, anchors, image_data, boxes, detectors_mask, matching_true_boxes, validation_split=0.1):
    ...
    model.save('trained_stage_1_model.h5')
    ...
    model.save('trained_stage_2_model.h5')
    ...
    model.save('trained_stage_3_model.h5')
